...
from open_spiel.python.bots import policy
...
_KNOWN_PLAYERS = [
    ...
    # MCCFR Bot with Outcome Sampling
    "mccfr"
]

flags.DEFINE_string("game", "tic_tac_toe", "Name of the game.")
flags.DEFINE_enum("player1", "mccfr", _KNOWN_PLAYERS, "Who controls player 1.")
    ...
    if state.is_chance_node():
      ...
      if current_player == 0:
        ...
        for i in range(10000):
            cfr_solver.iteration(state)
        rng = np.random.RandomState(FLAGS.seed)
        pol = cfr_solver.average_policy()
        action = policy.PolicyBot(0, rng, pol).step(state)
